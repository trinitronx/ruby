...
#define _RUBY_DEBUG_LOG(...) ruby_debug_log(__FILE__, __LINE__, __PRETTYFUNC__, "" __VA_ARGS__)
...
#define RUBY_DEBUG_LOG(...) do { \
    if (RUBY_DEBUG_LOG_ENABLED(__PRETTYFUNC__, __FILE__)) \
        ruby_debug_log(__FILE__, __LINE__, __PRETTYFUNC__, "" __VA_ARGS__); \
} while (0)

#define RUBY_DEBUG_LOG2(file, line, ...) do { \
    if (RUBY_DEBUG_LOG_ENABLED(__PRETTYFUNC__, file)) \
        ruby_debug_log(file, line, __PRETTYFUNC__, "" __VA_ARGS__); \
} while (0)
